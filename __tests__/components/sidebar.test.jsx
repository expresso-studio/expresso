...
describe("Sidebar & SidebarProvider", () => {
    test("renders with defaultOpen (expanded initially)", () => {
        ...
        expect(screen.getByTestId("content").closest("[data-state]")).toHaveAttribute("data-state", "expanded")
    })
    ...
    test("renders collapsed if defaultOpen={false}", () => {
        ...
        expect(screen.getByTestId("content").closest("[data-state]")).toHaveAttribute("data-state", "collapsed")
    })
...
describe("SidebarGroup family", () => {
    test("renders group, label, content, action", () => {
            ...
            <SidebarProvider>
                <Sidebar>
                    <SidebarGroup data-testid="group">
                        ...
                        <SidebarGroupContent data-testid="group-content">Group Content</SidebarGroupContent>
                        <SidebarGroupAction data-testid="group-action">Group Action</SidebarGroupAction>
                    </SidebarGroup>
                </Sidebar>
            </SidebarProvider>
        )
        ...
        expect(screen.getByTestId("group-content")).toHaveTextContent("Group Content")
        expect(screen.getByTestId("group-action")).toHaveTextContent("Group Action")
    })
})
                ...
                <Sidebar>
                    <SidebarMenu data-testid="menu">
                        <SidebarMenuItem data-testid="item">
                            <SidebarMenuButton data-testid="button" tooltip="Tooltip" isActive>
                                Menu Button
                            </SidebarMenuButton>
                            ...
                            <SidebarMenuSub data-testid="sub">
                                <SidebarMenuSubItem data-testid="subitem">
                                    <SidebarMenuSubButton data-testid="subbutton">Sub Button</SidebarMenuSubButton>
                                </SidebarMenuSubItem>
                            </SidebarMenuSub>
                        </SidebarMenuItem>
                    </SidebarMenu>
...
describe("SidebarMenuAction (showOnHover prop)", () => {
    test("applies hover classes when showOnHover is true", () => {
        ...
        expect(menuActionEl.className).toMatch(/group-focus-within\/menu-item:opacity-100/);
        expect(menuActionEl.className).toMatch(/group-hover\/menu-item:opacity-100/);
        expect(menuActionEl.className).toMatch(/data-\[state=open\]:opacity-100/);
        expect(menuActionEl.className).toMatch(/peer-data-\[active=true\]\/menu-button:text-sidebar-accent-foreground/);
...
describe("SidebarMenuSubButton", () => {
    test("renders as an anchor with correct attributes and conditional classes", () => {
        ...
        expect(subButton.className).toMatch(/data-\[active=true\]:bg-sidebar-accent/);
    });
});
...
describe("SidebarMenuAction - asChild prop", () => {
    test("renders as a button element when asChild is false (default)", () => {
        render(
            <SidebarMenuAction data-testid="action-button">
            Action
            </SidebarMenuAction>
        );
