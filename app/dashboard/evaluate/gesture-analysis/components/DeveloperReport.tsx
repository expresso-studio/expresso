...
const DeveloperReport: React.FC<DeveloperReportProps> = ({
  ...
  const getLandmark = (index: number): PoseLandmark | undefined => {
    return poseLandmarks && poseLandmarks.length > index
      ? poseLandmarks[index]
      : undefined;
  };
          ...
          <div className="space-y-4">
            ...
            <div className="mt-4">
              ...
              <div className="space-y-2">
                ...
                <LandmarkVisibility
                  name="Right Eye"
                  landmark={getLandmark(5)}
                />
                <LandmarkVisibility
                  name="Left Shoulder"
                  landmark={getLandmark(11)}
                />
                <LandmarkVisibility
                  name="Right Shoulder"
                  landmark={getLandmark(12)}
                />
                <LandmarkVisibility
                  name="Left Elbow"
                  landmark={getLandmark(13)}
                />
                <LandmarkVisibility
                  name="Right Elbow"
                  landmark={getLandmark(14)}
                />
                <LandmarkVisibility
                  name="Left Wrist"
                  landmark={getLandmark(15)}
                />
                <LandmarkVisibility
                  name="Right Wrist"
                  landmark={getLandmark(16)}
                />
                <LandmarkVisibility
                  name="Left Hip"
                  landmark={getLandmark(23)}
                />
                <LandmarkVisibility
                  name="Right Hip"
                  landmark={getLandmark(24)}
                />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-800 p-3 rounded-lg">
              <h4 className="text-sm font-medium text-gray-300 mb-2">
                Pose Landmarks{" "}
                {poseLandmarks.length > 0
                  ? `(First 5 of ${poseLandmarks.length})`
                  : "(None)"}
              </h4>
              <pre className="text-xs text-gray-400 overflow-x-auto">
                {poseLandmarks.length > 0
            ...
            <div className="text-xs text-gray-500 mt-2">
              Note: Only showing first 5 landmarks to avoid overwhelming the
              display.
            </div>
          </div>
        )}
