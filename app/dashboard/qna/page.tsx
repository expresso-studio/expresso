...
export default function Page() {
    ...
    const fetchData = async () => {
      ...
      try {
        const res = await retryFetch(() =>
          fetch(
            `/api/get-qna-info?userId=${encodeURIComponent(user.sub!)}&id=${presentationID}`,
          ),
        );
  ...
  return (
    <PageFormat breadCrumbs={[{ name: "qna" }]}>
      ...
      <p>
        Below are questions provided for you to practice answering questions the
        audience may have. <br />
        Feel free to click to see hints on how to approach answering the
        question.
      </p>
      <p></p>
      <div className="flex flex-col gap-4 h-screen">
        <div
          className={`flex items-center justify-center w-[min(100%,800px)]
                    max-h-[50px] min-h-[50px] bg-lightCaramel dark:bg-darkCaramel mb-0 p-4 mx-auto text-xl`}
        >
          <p>{title}</p>
        </div>
        <div
          className={`w-[min(100%,800px)] h-[min(100%,500px)] mx-auto p-16 
