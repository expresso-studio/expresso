...
if (!apiKey || !serviceUrl) {
  console.error(
    "Missing IBM Watson API Key or Service URL in environment variables.",
  );
...
export async function POST(request: NextRequest) {
  if (!apiKey || !serviceUrl) {
    return NextResponse.json(
      { error: "IBM Watson service not configured." },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { emotion: emotionResult, sentiment: sentimentResult },
      { status: 200 },
    );
  } catch (error: unknown) {
