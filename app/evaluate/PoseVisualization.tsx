...
const PoseVisualization: React.FC<PoseVisualizationProps> = ({ results }) => {
  ...
  useEffect(() => {
    ...
    if (newPositions.length > 0) {
      ...
      setHandPositions((prev) => {
        const filtered = [...prev, ...newPositions].filter(
          (pos) => currentTime - pos.timestamp < 1000
        );
  ...
  const CustomTooltip: React.FC<CustomTooltipProps> = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      const getPositionDescription = (x: number, y: number) => {
        const horizontal = x < 0.33 ? "left" : x < 0.66 ? "center" : "right";
        const vertical = y > 0.66 ? "top" : y > 0.33 ? "middle" : "bottom"; // Adjusted for inverted Y
      ...
      return (
        <div className="bg-black/90 p-3 rounded border border-gray-600">
          ...
          <p className="text-gray-300">{timeDiff}s ago</p>
        </div>
      );
    }
    return null;
  ...
  return (
    <div
      className="absolute bottom-0 left-0 w-full bg-black/90 p-4"
      style={{ height: "300px" }}
    >
      <div className="flex flex-col h-full">
        <div className="mb-4">
          ...
          <div className="text-gray-400 text-sm mt-1">
            Showing the last second of hand movements. Larger dots are more
            recent positions.
          </div>
        </div>
        <div className="flex-1 bg-black/50 rounded">
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
              ...
              >
                <Label
                  value="Camera View (Left to Right)"
                  position="bottom"
                  offset={0}
                  fill="#fff"
                />
              ...
              <YAxis
                ...
                ticks={[0, 0.5, 1]}
              ></YAxis>
              <ZAxis type="number" dataKey="timestamp" range={[100, 1000]} />
